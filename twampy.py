    ...
    def run(self):
        ...
        while self.running:
            while select.select([self.socket], [], [], 0)[0]:
                ...
                print(f"Reply from {address[0]} [rseq={rseq} sseq={sseq} rtt={delayRT:.4f}ms outbound={delayOB:.4f}ms inbound={delayIB:.4f}ms]")
    ...
    def run(self):
        ...
        while self.running:
            try:
                ...
                #print(f"Reply from {address[0]} [sseq={sseq} outbound={1000 * (t2 - t1):.2f}ms]")

                #print(f"Request from %s:%d [sseq=%d outbound=%.2fms]", address[0], address[1], sseq, 1000 * (t2 - t1))
