...
RECEIVER_IP=16.170.238.74 #Change to applicable receiver address
...
#MEDIA=~/bbb_sunflower_1080p_30fps_normal.mp4
#gst-launch-1.0 rtpbin name=rtpbin multifilesrc location=$MEDIA location=$MEDIA loop=true stop-index=-1 ! qtdemux name=demux ! queue ! h264parse ! avdec_h264 ! videoconvert ! x264enc key-int-max=30 ! rtph264pay mtu=1300 ! udpsink host=127.0.0.1 port=30000 &

MEDIA=~/bbb_sunflower_1080p_30fps_normal.mp4
gst-launch-1.0 rtpbin name=rtpbin multifilesrc location=$MEDIA location=$MEDIA loop=false stop-index=-1 ! qtdemux name=demux ! queue ! h264parse ! avdec_h264 ! videoconvert ! x264enc tune=zerolatency bitrate=1000000 ! queue max-size-buffers=2 ! rtph264pay mtu=1300 ! udpsink host=127.0.0.1 port=30000 &
